...
|---|---|---|
| 2026-08-19 | [PLAN_sidecar_product.md](../research/PLAN_sidecar_product.md) | The distribution story, end to end: the build recipe as DATA (`build-recipes.json`, with a test that keeps it honest against the manifest in both directions, and build times that are measured or absent — never guessed); the verification gate on the wire (`/health.self_check`, two thresholds because "may serve" and "is trustworthy" are two questions, measured at 1.0000004); the licence position resolved from the artefacts, which turned up a shipped **MPL-2.0** dependency four levels down that nobody had noticed; a generator for the canary's own oracle, since the original script never left the monorepo; a tag-driven release workflow for the one flavour that may legally ship; and an opt-in idle unload. Its own phase-4 audit found three of four ergonomics items already done and the plan text stale — its console half was closed the same day in `dew_flow_rag_qln` (the runtime panel renders the cosine beside the active provider), and the last item — the release tag — was taken 2026-08-20/21. **The tag turned out to be an instrument**: `v0.1.0` had been cut two commits before `/health.host` existed (`v0.1.1`), and verifying the arm on real hardware found its second segment answering `auto` — the request published as the outcome, on the DEFAULT configuration, while DirectML was demonstrably serving (`v0.1.2`). Every unit test passed over both |
