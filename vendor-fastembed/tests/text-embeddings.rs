...
fn test_user_defined_reranking_large_model() {
    ...
    let model_repo = api.model("perlets/bge-reranker-v2-m3".to_string());

    // Download the onnx model file
    let onnx_file = model_repo.download("onnx/model.onnx").unwrap();
    // Onnx model exceeds the limit of 2GB for a file, so we need to download the data file separately
    let _onnx_data_file = model_repo.get("onnx/model.onnx_data").unwrap();
