...
/// VENDORED PATCH (clauderag): whether a token-level output tensor can be indexed at every
/// `(batch, seq)` position the INPUT has.
///
/// `post_process_bgem3` bounds its loops by `input_ids.shape()` but slices `hidden_states`, so an
/// output smaller than the input runs off the end and ndarray panics with
/// `assertion failed: index < dim`. That panic fires while the engine mutex is HELD, poisoning it, and
/// the sidecar then rebuilds a perfectly healthy session on every following request.
///
/// Observed under the MIGraphX EP: the FIRST `session.run` on a freshly built session returns a short
/// batch — `[16, 1024, 1024]` for a 64x1024 input — while every later run is correct.
/// `>=` rather than `==` on purpose: a LARGER output is indexable and therefore safe; only a smaller
/// one is not.
pub(crate) fn output_covers_input(shape: &[usize], batch_size: usize, encoding_length: usize) -> bool {
    shape.len() == 3 && shape[0] >= batch_size && shape[1] >= encoding_length
}
...
impl SparseTextEmbedding {
        ...
        let output = texts
            ...
            .map(|batch| {
                ...
                let embeddings = match self.model {
                    ...
                    SparseModel::BGEM3 => {
                        // VENDORED PATCH (clauderag): the bge-m3 ONNX export carries TWO outputs —
                        // `sentence_embedding` [batch, hidden] and `token_embeddings`
                        // [batch, seq, hidden]. post_process_bgem3 needs the TOKEN-level tensor, but
                        // `keys().next()` picks whichever the runtime lists first: right on
                        // CPU/DirectML, wrong under the MIGraphX EP, whose ordering puts the 2-D
                        // pooled output first — the per-(batch, seq) slice then panics with
                        // `assertion failed: index < dim` and poisons the engine. Select by NAME,
                        // falling back to the first 3-D output, and error instead of panicking.
                        let output_key = match outputs.keys().find(|k| k.contains("token_embedding"))
                        {
                            Some(key) => key,
                            None => outputs
                                .iter()
                                .find(|(_, value)| {
                                    value
                                        ...
                                        .is_ok_and(|(shape, _)| shape.len() == 3)
                                })
                                .map(|(key, _)| key)
                                .ok_or_else(|| {
                                    anyhow::anyhow!(
                                        "no token-level (3-D) output among the model outputs: {:?}",
                        ...
                        // VENDORED PATCH (clauderag), second one: post_process_bgem3 bounds its loops
                        // by the INPUT (batch_size x encoding_length) but indexes the OUTPUT, so a
                        // tensor smaller than the input slices out of bounds and ndarray panics with
                        // `assertion failed: index < dim`. That panic happens while the engine mutex
                        // is HELD, so it poisons it, and lock_healing then throws a perfectly healthy
                        // session away and pays a full ~60s rebuild on the next request — forever,
                        // because the next request panics too. Observed under the MIGraphX EP with a
                        // correctly selected (by name) 3-D output.
                        // Fail the request instead, naming BOTH shapes: an error is a clean 500 that
                        // leaves the session intact, and it is the only way to see what the runtime
                        // actually returned.
                        anyhow::ensure!(
                            output_covers_input(&shape, batch_size, encoding_length),
                            "bge-m3 sparse output `{}` came back shaped {:?}, which does not cover \
                             the input batch x seq of {}x{} — the runtime returned a mis-shaped \
    ...
    /// VENDORED VISIBILITY CHANGE (clauderag): `pub(crate)` so [`crate::bgem3_dual`] runs the SAME
    /// lexical head over the token tensor its single session produced — a re-implementation there
    /// would be exactly the formula-drift risk the unified session exists to avoid.
    pub(crate) fn post_process_bgem3(
...
/// VENDORED PATCH (clauderag): the shape guard that turns a mis-shaped runtime output into a failed
/// request instead of a panic. Pure, so the contract is testable without a model or a GPU.
#[cfg(test)]
mod clauderag_shape_guard_tests {
    use super::output_covers_input;

    /// The measured MIGraphX defect: 64x1024 asked for, `[16, 1024, 1024]` returned. Every index from
    /// batch 16 upwards would slice out of bounds, so this MUST be rejected.
    #[test]
    fn a_short_batch_dimension_is_rejected() {
        assert!(!output_covers_input(&[16, 1024, 1024], 64, 1024));
    }

    /// The same failure on the sequence axis: the loop runs to the input's seq length regardless.
    #[test]
    fn a_short_sequence_dimension_is_rejected() {
        assert!(!output_covers_input(&[64, 512, 1024], 64, 1024));
    }

    /// The healthy case — exactly the shape that was asked for.
    #[test]
    fn a_matching_shape_is_accepted() {
        assert!(output_covers_input(&[64, 1024, 1024], 64, 1024));
    }

    /// A LARGER output is still indexable at every position the loops visit, so rejecting it would fail
    /// requests the post-process can serve perfectly well.
    #[test]
    fn an_oversized_output_is_accepted() {
        assert!(output_covers_input(&[128, 2048, 1024], 64, 1024));
    }

    /// The ORIGINAL bug this file was first patched for (d4e1266): the 2-D pooled `sentence_embedding`
    /// was selected instead of the 3-D `token_embeddings`, and `s![batch, seq, ..]` panicked on rank.
    /// Selecting by name fixed the choice; this keeps the rank itself checked.
    #[test]
    fn a_two_dimensional_pooled_output_is_rejected() {
        assert!(!output_covers_input(&[64, 1024], 64, 1024));
    }

    /// A degenerate empty batch must not be treated as covering anything.
    #[test]
    fn an_empty_output_is_rejected() {
        assert!(!output_covers_input(&[0, 0, 0], 64, 1024));
    }
}
