...
| `ORT_PROVIDER` | *(empty)* | `auto` \| `cuda` \| `dml` \| `migraphx` \| `cpu`. Empty ⇒ the first request's `provider` hint (the C# side forwards the operator's Settings → Models choice), else `auto`. **`auto` is resolved by trying**, not by handing ort a soft list: this binary's compiled GPU providers are attempted in chain order (cuda → migraphx → dml), each fail-hard, then CPU — and whichever one built the session is what `/health.active_provider` reports. Before v0.1.2 it reported the literal `auto`, which named no hardware. Explicit choices resolve to themselves with no fallback, so a broken GPU setup stays visible instead of being quietly served by the CPU. |
