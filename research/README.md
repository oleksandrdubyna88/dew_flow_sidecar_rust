...
| [PLAN_sidecar_product.md](PLAN_sidecar_product.md) | Design record, IMPLEMENTED 2026-08-19 — the distribution story: the build recipe as data with a test that keeps it honest against the manifest, the verification gate on the wire (two thresholds, because "may serve" and "is trustworthy" are different questions), the licence position resolved from the artefacts (which found a shipped MPL-2.0 dependency nobody had noticed), a generator for the canary's own oracle, the release workflow for the one flavour that may ship, and an opt-in idle unload. Its phase-4 audit found three of four items already done and the plan text stale |
